...
class Canvas extends Component {
	...
	async fileUploadHandler(img) {
		...
		this.setState({
			prediction: Buffer.from(response.data, 'binary').toString('base64'),
			predictionProgress: '',
		});
	...
	sketch = p => {
		...
		p.setup = () => {
			...
			canvas = p.createCanvas(this.state.canvasLength, 300);
			p.noStroke();
	...
	render() {
		...
		return (
			<div className="canvas">
				...
				>Generate Font{predCountdown}</button>
				</div>
				{this.state.predictionProgress ? (
					...
					<img src={`data:image/jpeg;base64,${this.state.prediction}`}/>						
					)}
